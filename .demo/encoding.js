...
// Encryption
// bcrypt.hash('Laurel', 30).then(hashedString => {
//   console.log('HERE IS OUR HASHED STRING: ', hashedString);

// // comparison
//   bcrypt.compare('Laurel', `${hashedString}23874619284756`).then(isTheSame => {
...
async function main() {
  let encryptedHash = await encrypt('Laurel');
  let isTheSame = await decrypt('Laurel', encryptedHash);
